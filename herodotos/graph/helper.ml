...
let compute_bug_info vb1 vb2 prefix fel vlist scmpath vminopt bug =
  ...
  in
  let clean_c = Org.clean_link_text prefix v fb pos c in
    (author, fb, clean_c, fmin, bmin, bmax, fmax)
...
let crop varray atts curve values =
  ...
  let d_list = Config.get_pattern false atts pattern catts in
    match d_list with
	[d] ->
	  begin
...
let build_evolution_for_org vb debug name grinfo scmfeature atts curve allbugs evolfunc project patt info =
  ...
  let prj = Config.get_project (catts@atts) project in
    try
      let bugset = List.hd (Config.get_bugset_of_curve atts curve) in
	...
	try
	  ...
	  let pdir = Config.get_prjdir prj in
	  let prefix = !Setup.projectsdir ^ pdir ^"/" in
	    compute_graph vb debug prefix fel bl varray scmpath vminopt;
	with FileNotFound file ->
	  prerr_endline ("*** ERROR *** Unable to find "^file^" when generating "^name^
			   ...
			   "\nTry 'rm "^prj^Global.sep^patt^Global.existext^"' to refresh");
	  []
      in
...
(* 		 let evolfunc = if ytype = "usersize" then Size.evolution else evolfunc in *)
	  ...
	  let format = Config.get_format patt in
	    ...
	    match format with
		Ast_config.Org ->
		  let info = (linetype, color, marktype, marksize, label) in
		    build_evolution_for_org vb debug name grinfo scmfeature atts curve allbugs evolfunc project patt info

	      | Ast_config.CSV  ->
(*
		  let prj = Config.get_project atts project in
 		    try *)
		      let bugset = List.hd (Config.get_bugset_of_curve atts curve) in
		      let info = snd (List.find (by_bugset bugset) allbugs) in
		      let (varray, fel, bl, _) = info in
  ...
  Debug.profile_code "Helper.build_evolution_of_size"
    (fun () ->
	 ...
	 match curve with
	   ...
	   | (None, None, catts, _) ->
	       let ctxt = (vb, debug, name, grinfo, scmfeature, atts, curve, [], evolfunc) in
	       let data = eval_data debug ctxt (Config.get_data_exp catts) in
		 begin
		   match data with
		       Empty   -> None
		     | Cst _   -> None
...
let build_evolution vb debug name grinfo scmfeature atts curve allbugs evolfunc : value option =
  Debug.profile_code "Helper.build_evolution"
    (fun () ->
       ...
       let patt_list = Config.get_pattern false atts pattern catts in
	 match patt_list with
	   ...
	   | _ ->
	       let ctxt = (vb, debug, name, grinfo, scmfeature, atts, curve, allbugs, evolfunc) in
	       let data = eval_data debug ctxt (Config.get_data_exp catts) in
		 match data with
		     Empty   -> None
		   | Cst _   -> None
