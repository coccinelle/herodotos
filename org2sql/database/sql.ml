...
let insert_annot prefix cfile cver cpos org =
  let Ast_org.Org(l, s, r, link, sub) = org in
    if l = max_int then
      let (file, ver, pos, t) = Org.flat_link prefix 1 link in
...
let print_org ch prefix orgfile org =
  ...
  in
  let orgstr = insert_correl_report prefix basefile patt org in
 ...
 *********************************************
 *)
let all = Str.regexp_string "runall"
...
let insert_note prefix src patt org =
  ...
  let (line, colb, cole) = pos in
  let file_id = "get_file('"^ver^"', '"^Misc.get_canonical_name prefix ver file^"')" in
  let newpatt = if Str.string_match all patt 0 then text else patt in
  let values = String.concat ", "
    [file_id;
     s2s src;
     s2s newpatt;
...
let print_org_as_note ch prefix orgfile org =
  let basefile = Filename.basename orgfile in
  ...
  let value = insert_note prefix basefile patt org in
